...
describe RepositoryService do
  ...
  describe '#create_repository' do
    subject(:repository) { service.update_or_create_repository!(product, url, attributes, custom:).reload }
    ...
    context 'URLs of SCC repositories changes' do
      subject(:repository) do
        service.update_or_create_repository!(product, old_url, attributes, custom:)
        expect(Repository.find_by(external_url: old_url)).not_to eq(nil)

        service.update_or_create_repository!(product, url, attributes, custom:).reload
      end
    ...
    context 'self heals SCC repos' do
      subject(:repository) do
        service.update_or_create_repository!(product, url, attributes, custom:).update(scc_id: old_scc_id)
    ...
    context 'custom repo with same url' do
      subject(:repository) do
        service.update_or_create_repository!(product, url, attributes, custom:).update(scc_id: nil)
        service.update_or_create_repository!(product, url, attributes, custom:).reload
      end
