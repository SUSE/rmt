class Profile < ApplicationRecord
  ...
  class << self
    ...
    def with_unique_type_and_identifier(key_tuples)
    ...
    def identify_known_profiles(profiles)
    ...
    def ensure_complete_profiles_exist(profiles)
      profiles.map do |ptype, pinfo|
        ensure_profile_exists(ptype, pinfo)
      end
    end

    def ensure_profile_exists(ptype, pinfo)
      ...
      # NOTE: To avoid deadlocks we need to use LOCK IN SHARE MODE to
