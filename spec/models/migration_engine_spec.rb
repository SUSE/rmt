  ...
  describe '#generate' do
    ...
    context 'with base plus extension products' do
      ...
      context 'with simple dependencies' do
        ...
        its(:first) { is_expected.to contain_exactly(sle12sp1, sleha12sp1, sdk12sp1) }
      end
      ...
      context 'when only the extension can get upgraded' do
        ...
        its(:first) { is_expected.to contain_exactly(sle12, sdk12, cloud9) }
        its(:second) { is_expected.to contain_exactly(sle12, sdk12, cloud8) }
      end
      ...
      context 'when not all products are upgradeable' do
        ...
        it 'raises error with help message' do
          expect { migrations }.to raise_error do |error|
            expect(error).to be_a(MigrationEngine::MigrationEngineError)
            expect(error.message).to match(%r{There are activated extensions/modules on this system that cannot be migrated})
          end
        end
      end
