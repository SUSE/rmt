...
We would like to have a single entry-point script named `rmt` to provide CLI to RMT functions.
...
|  |  |
|  |  +--> -a, [--all], [--no-all] (Show unreleased products, default off)
|  |  +--> -r, [--repository-status], [--no-repository-status] (Show if all madatory repositories were mirrored)
|  |
...
|  |
|  +--> add_repo PRODUCT TARGET (Add a custom repo to the product)
|  |
|  +--> remove_repo PRODUCT TARGET (Remove a custom repo from the product)
|
+--+ repos
|  |
|  +--+ list (List repositories which are enabled for mirroring)
|  |  |
...
|  |  |
|  |  +--> -i, [--include-non-mandatory], [--no-include-non-mandatory] (Enable non-mandatory repos when enabling by product, default off)
|  |
|  +--> disable TARGET (Disable repositories, TARGET is either product string (identifier/version/arch) or repo ID)
|  |
|  +--> create NAME URL (Create a custom repo)
|  |
|  +--> edit (Edit an existing custom repo)
|  |
|  +--> delete TARGET (Delete a custom repo by name, ID or URL)
|
+--+ mirror (Mirror all repositories that are enabled)
|  |
|  +--> -r, [--repository] REPOSITORY_ID (Mirror only specified repository)
|  +--> -u, [--uri] URL PATH (Mirror the repository from specified URL to specified path)
|
...
# rmt scc help
Commands:
  rmt scc sync     # Synchronize database with SCC

  -o, [--out-file] FILENAME (Write SCC data into a file rather than into the DB)
  -i, [--in-file] FILENAME (Load results from a file rather than from SCC)

Run 'rmt scc help COMMAND' for more information on a command.
```
...
```
+------+------------------------------------------------+---------+--------------+----------+--------------------+
| ID   | Name                                           | Version | Architecture | State    | Product string     |
+------+------------------------------------------------+---------+--------------+----------+--------------------+
| 1172 | SUSE Linux Enterprise Software Development Kit | 10      | x86_64       | released | sles-sdk/10/x86_64 |
+------+------------------------------------------------+---------+--------------+----------+--------------------+
```
...
```
+------+------------------+------------------------------------------------+---------+---------------+
| ID   | Name             | Description                                    | Mirror? | Last mirrored |
+------+------------------+------------------------------------------------+---------+---------------+
| 1712 | SLE-HA12-Updates | SLE-HA12-Updates for sle-12-x86_64             | true    | 2017-01-01    |
+------+------------------+------------------------------------------------+---------+---------------+
```
